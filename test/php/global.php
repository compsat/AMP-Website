<?php
  ...
  define('FILENAME', basename($_SERVER['SCRIPT_FILENAME'], '.php'));
	define('ROOT_DIRECTORY', substr(dirname(dirname(__FILE__)),
  ...
  function absolute_path($path) {
    ...
    if ($root == '/' || $root == '')
      ...
      $root = "/$root/";
    return $root.(starts_with($path, '/') ? substr($path, 1) : $path);
  }
  ...
  /**
   * Saves or reads JSON file in the /json directory. Uses /json_dev
   * directory if in local server.
   * @param string $filename the JSON file name that will be searched in the
   *                         /json or /json_dev directory. will automatically
   *                          append the .json extension.
   * @param string &$data    data to put to the JSON file. if not specified,
   *                         this function will return the contents of the json
   *                         file.
  ...
  function json($filename, $data = null) {
    ...
    if ($prefix[strlen($prefix) - 1] != '/')
      $prefix .= '/';
    $filename = $prefix.JSON_DIR."/$filename.json";
    if (!is_null($data)) {
  ...
  function source($link) {
    ...
    if ($ext === 'css') { ?>
      <link rel='stylesheet' type='text/css' href='<?php echo $link; ?>?ver=<?= VERSION ?>'>
    <?php } else if ($ext === 'js') { ?>
      <script src='<?php echo $link; ?>?ver=<?= VERSION ?>'></script>
    <?php } else if ($ext === 'php') {
      require_once $link;
    }
  }
  ...
  function globalLinks() { ?>
    <base href="<?= ROOT_DIRECTORY ?>"/>
    <meta charset='utf-8'>
    <meta http-equiv='X-UA-Compatible' content='IE=edge'>
    <!--<meta name='viewport' content='maximum-scale=0.5, initial-scale=1'>-->
    <meta name='viewport' content='maximum-scale=1, initial-scale=1'>
    <meta property="og:image" content="<?= absolute_path('images/icons/logo-black.png') ?>">
    ...
    <link rel="shortcut icon" href="<?= absolute_path('images/icons/logo.ico') ?>" type="image/x-icon">
    <link rel="icon" href="<?= absolute_path('images/icons/logo.ico') ?>" type="image/x-icon">
    <script>window.abspath = function(path) {
      if (path.length > 0 && path[0] == '/')
        path = path.substr(1);
      <?php if (ROOT_DIRECTORY == '/' || ROOT_DIRECTORY == '') { ?>
        return '/' + path;
      <?php } else { ?>
  ...
  function extractMenu(&$menu, $root = true) {
    foreach ($menu as &$item) {
      ...
      $href = get($item, 'active') && get($item, 'href') ? "href='".(starts_with($item['href'], '/') ? absolute_path($item['href']) : $item['href'])."'" : "";
    ...
    if ($root && authenticated())
      echo "<div id='admin-menu-item' class='node'><a href='".absolute_path('/admin')."''>admin</a></div>";
  }

  function createAdminBackground() { ?>
    <section class='fixed amp-bg' src='/<?= ROOT_DIRECTORY.'/'.relative_path(get(json('meta'), 'admin-bg')) ?>'></section>
  <?php }
