...
$site = $protocol.$host.absolute_path('/bands.php');
function template($item, $id = null) {
	...
	<div class='node' <?= $id ? "id='$id'" : '' ?>>
		<div class='legend'>
			...
			<img class='img uploadable' name='image' src='<?= absolute_path(get($item, 'image')) ?>'>
			...
			<tr>
				...
				<td>
					...
					<img class='img uploadable' name='image strip' src='<?= absolute_path(get($item, 'image strip')) ?>'>
				</td>
			</tr>
			</table>
		</div>
